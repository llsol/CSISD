...
# ----------------------------
# 5) Pitch with stability segments (Polars) — standardized names
# ----------------------------
#def plot_pitch_with_stability_segments(
#    df: pl.DataFrame,
#    *,
#    time_col: str = "time_rel_sec",
#    pitch_col: str = "f0_savgol_p3_w13",
#    stable_col: str = "stable_region",
#    title: str | None = None,
#    time_range: tuple[float, float] | None = None,   # renamed from zoom_between
#    show_threshold_lines: bool = True,
#    pitch_unit: str = "cents",
#):
#    """
#    Colors segments in crimson when stable_col is True, else lightgrey.
#    """
...
#    else:
#        dfp = df
#        t_start = float(dfp[time_col].min())
#        t_end = float(dfp[time_col].max())
#
#    x = dfp[time_col].to_numpy()
#    y = dfp[pitch_col].to_numpy()
#
#    if stable_col in dfp.columns:
#        stable = dfp[stable_col].fill_null(False).to_numpy()
#    else:
#        stable = np.zeros(len(dfp), dtype=bool)
#
#    segments = []
#    colors = []
#    for i in range(len(dfp) - 1):
#        if np.isnan(y[i]) or np.isnan(y[i + 1]) or np.isnan(x[i]) or np.isnan(x[i + 1]):
#            continue
#        segments.append([(x[i], y[i]), (x[i + 1], y[i + 1])])
#        colors.append("crimson" if bool(stable[i]) else "lightgrey")
#
#    lc = LineCollection(segments, colors=colors, linewidths=1.8)
#
#    fig_width = 16
...
#    ax.set_xlim(t_start, t_end)
#
#    y_min = float(np.nanmin(y)) if np.any(np.isfinite(y)) else 0.0
#    y_max = float(np.nanmax(y)) if np.any(np.isfinite(y)) else 1.0
#    pad = 50 if pitch_unit == "cents" else 0.0
#    ax.set_ylim(y_min - pad, y_max + pad)
#
#    ax.set_xlabel("Time (s)")
...
#    """
#
#    # --- Svara system definition (Carnatic-style) ---
#    svara_labels = ["S", "R2", "G2", "M1", "P", "D1", "D2", "N2", "Ṡ"]
#    intervals = [0, 200, 100, 200, 200, 100, 100, 100, 200]  # cents
#    base_cents = np.cumsum(intervals)
#
#    cents_positions = []
#    svaras_full = []
...
#    # --- Plot ---
#    plt.figure(figsize=(14, 4))
#    plt.plot(pitch_cents_grid, density, label="Pitch KDE")
#    plt.plot(peak_cents, peak_densities, "ro", label="Detected peaks")
#
#    plt.title("Peak pitch density detection (svarasthana candidates)")
...
#    plt.xticks(cents_positions, svaras_full, fontsize=9)
#
#    # optional annotation of detected svara peaks
#    if svarasthana_df is not None and "cents" in svarasthana_df.columns:
#        for svara in svarasthana_df["cents"].to_numpy():
#            idx = np.abs(pitch_cents_grid - svara).argmin()
#            plt.text(
#                svara,
#                density[idx],
#                f"{svara:.0f}",
...
#    *,
#    time_col: str = "time_rel_sec",
#    pitch_col: str = "f0_spline_savgol_13_cents",
#    stable_col: str = "13_stable_region",
#    title: str | None = None,
#    zoom_between: tuple[float, float] | None = None,
#    show_threshold_lines: bool = True,
#    pitch_unit: str = "cents",  # "cents" or "hz" (only affects axis label)
#):
#    """
#    Polars version of your stability plot.
#    Colors segments in crimson when stable_col is True, else lightgrey.
#    """
#    if pitch_unit not in ("cents", "hz"):
#        raise ValueError("pitch_unit must be 'cents' or 'hz'")
...
#    lc = LineCollection(segments, colors=colors, linewidths=1.8)
#
#    # figure scaling similar to yours
#    fig_width = 16
#    dpi = 100
#    if (t_end - t_start) < 1.0:
#        fig_width = 14
...
#    ax.grid(True, linestyle="--", alpha=0.5)
#
#    # If you're in cents, show svaras as y-axis ticks (same as your original)
#    if pitch_unit == "cents":
#        svara_labels = ["S", "R2", "G2", "M1", "P", "D1", "D2", "N2", "S"]
#        intervals = [0, 200, 100, 200, 200, 100, 100, 100, 200]
#        base_cents = np.cumsum(intervals)
...
# ----------------------------
# 3) Pitch with flat/stable regions (Polars)
# ----------------------------
#def plot_pitch_with_flat_regions(
#    df: pl.DataFrame,
#    *,
...
#        segments.append([(x[i], y[i]), (x[i + 1], y[i + 1])])
#
#        # identical logic to your latest version: stable if both points are stable
#        is_stable_seg = bool(stable[i]) and bool(stable[i + 1])
#        colors.append("crimson" if is_stable_seg else "lightgrey")
#
#    if not segments:
...
#    ax.grid(True, linestyle="--", alpha=0.5)
#
#    # svaras at Y-axis (only meaningful in cents)
#    if pitch_unit == "cents":
#        svara_labels = ["S", "R2", "G2", "M1", "P", "D1", "D2", "N2", "S"]
#        intervals = [0, 200, 100, 200, 200, 100, 100, 100, 200]
#        base_cents = np.cumsum(intervals)
