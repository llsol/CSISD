"""
Fine-tune SwiftF0 on the SCMS Carnatic dataset.

Reproduces the Plaja-Roglans et al. (2023) fine-tuning approach
(originally applied to FTA-Net) using SwiftF0 as the base model.

Loss:
    voicing BCE  — binary cross-entropy on voiced/unvoiced decision
    pitch CE     — cross-entropy over 200 pitch bins (voiced frames only)
    total = voicing_weight * voicing_bce + pitch_ce

Usage:
    python -m src.pitch_extraction.swiftf0_finetune.train
    python -m src.pitch_extraction.swiftf0_finetune.train --freeze-conv

To extend a completed run with more epochs, just pass a higher --epochs value;
the resume logic will continue from the last checkpoint:
    python -m src.pitch_extraction.swiftf0_finetune.train --epochs 80

Checkpoints (never overwrite previous runs):
    data/interim/models/swiftf0_carnatic/run_001/epoch_001.pt   (per epoch)
    data/interim/models/swiftf0_carnatic/run_001/last.pt        (resume target)
    data/interim/models/swiftf0_carnatic/run_001/best.pt        (best val loss)
    data/interim/models/swiftf0_carnatic/run_001/history.json   (full log)
    data/interim/models/swiftf0_carnatic/swiftf0_carnatic.onnx  (exported on completion)
"""
...
import settings
from src.pitch_extraction.swiftf0_finetune.model import SwiftF0, N_PITCH_BINS
...
EPOCHS         = 50
LR             = 1e-4
CROP_SEC       = 0.0    # 0 = full chunk (~30 s); use --crop-sec N to override
VOICING_WEIGHT = 1.0    # relative weight of voicing BCE vs pitch CE
SEED           = 42
# ─────────────────────────────────────────────────────────────────────────────
...
def hz_to_bin(f0_hz: torch.Tensor, bin_centers: torch.Tensor) -> torch.Tensor:
    """
    Convert f0 in Hz to the index of the nearest pitch bin (log-space).
    f0_hz     (B, T)   — Hz, unvoiced frames are ignored by the caller
    bin_centers (200,) — sorted Hz values
    Returns: (B, T) long tensor of bin indices
    """
    log_f0      = torch.log(f0_hz.unsqueeze(-1).clamp(min=1.0))   # (B, T, 1)
    log_centers = torch.log(bin_centers.clamp(min=1.0))            # (200,)
    dist        = (log_f0 - log_centers).abs()                     # (B, T, 200)
    return dist.argmin(dim=-1)                                      # (B, T)


def pitch_cross_entropy(
    pitch_probs: torch.Tensor,   # (B, 200, T) Softmax output
    f0_hz:       torch.Tensor,   # (B, T)
    voiced:      torch.Tensor,   # (B, T) bool
    bin_centers: torch.Tensor,   # (200,)
) -> torch.Tensor:
    """Cross-entropy loss on voiced frames only."""
    if not voiced.any():
        return pitch_probs.sum() * 0.0

    target = hz_to_bin(f0_hz, bin_centers)          # (B, T)
...
def run_epoch(
    ...
    for batch in loader:
        ...
        with torch.set_grad_enabled(train):
            ...
            # Voicing loss — confidence is prob-mass within ±9 bins of argmax
...
def main():
    ...
    parser.add_argument("--freeze-conv", action="store_true",
                        help="Freeze conv stack; fine-tune only pitch projection")
    parser.add_argument("--epochs",     type=int,   default=EPOCHS)
    parser.add_argument("--batch-size", type=int,   default=BATCH_SIZE)
    parser.add_argument("--lr",         type=float, default=LR)
    parser.add_argument("--crop-sec",   type=float, default=CROP_SEC,
                        help="Random crop duration in seconds (0 = full chunk)")
    ...
    # ── dataset — official artist-based split ────────────────────────────
    train_stems, test_stems = scms_official_split(scms_root)
    train_ds = SCMSDataset(scms_root, split=train_stems, crop_sec=args.crop_sec)
    ...
    model.load_onnx_weights()

    if args.freeze_conv:
        model.freeze_conv()
        print("[train] Conv stack frozen — fine-tuning pitch projection only")
    ...
    # ── run directory (never overwrites previous runs) ───────────────────
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        train_ds.epoch = epoch   # seed random crops for this epoch
    ...
    model.export_onnx(CKPT_DIR / "swiftf0_carnatic.onnx")
